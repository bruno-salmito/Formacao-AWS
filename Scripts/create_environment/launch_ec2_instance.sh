...
# Script para lançar uma instância EC2
...
if ! command -v jq &> /dev/null; then
    echo ">[ERRO] jq não está instalado. Instale com: sudo apt-get install jq (Ubuntu/Debian) ou sudo yum install jq (RHEL/CentOS)"
    exit 1
fi
...
instance_info=$(aws "${aws_args[@]}")

# Verificar se o comando foi executado com sucesso
if [ $? -ne 0 ]; then
    echo ">[ERRO] Falha ao executar comando AWS CLI"
    exit 1
fi
