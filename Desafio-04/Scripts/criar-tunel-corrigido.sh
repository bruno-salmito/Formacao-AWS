#!/bin/bash
#-----------------------------------------------------
# Script: criar-tunel-corrigido.sh
...
function validate_instance() {
    ...
    echo -n "Verificando instância '$instance_name'... "
    ...
    if [ -z "$instance_id" ] || [ "$instance_id" = "None" ]; then
        ...
        echo -e "${RED}Instância '$instance_name' não encontrada ou não está em execução${RESET}"
        log_message "Erro: Instância '$instance_name' não encontrada ou não está em execução"
        return 1
    else
        echo -e "${GREEN}[OK]${RESET}"
        echo -e "${GREEN}Instance ID: $instance_id${RESET}"
        log_message "Instância validada: $instance_name ($instance_id)"
        echo "$instance_id"
        return 0
    fi
}
...
function create_simple_tunnel() {
    ...
    echo ""
    echo -e "${CYAN}Para parar o túnel, pressione Ctrl+C${RESET}"
    echo ""
    
    log_message "Iniciando túnel simples: $ec2_name ($instance_id) - Local:$local_port -> Remote:$remote_port"
    
    # Comando corrigido - removendo aspas duplas problemáticas
    ...
    if [ $exit_code -eq 0 ]; then
        log_message "Túnel simples finalizado normalmente"
    else
        echo -e "${RED}[ERRO] Falha ao criar túnel (código: $exit_code)${RESET}"
        log_message "Erro ao criar túnel simples (código: $exit_code)"
    fi
}
...
function create_rds_tunnel() {
    ...
    echo -e "${CYAN}=== TÚNEL PARA RDS (via bia-dev) ===${RESET}"
    echo ""
    
    # Valida a instância bastion
    bastion_id=$(validate_instance "$BASTION_INSTANCE")
    if [ $? -ne 0 ]; then
        echo -e "${RED}[ERRO] Instância bastion '$BASTION_INSTANCE' não disponível${RESET}"
        return 1
    fi
    
    # Solicita informações do RDS
    echo -n "Digite o endpoint do RDS: "
    read -r rds_endpoint
    ...
    # Cria o túnel
    echo ""
    echo -e "${YELLOW}Criando túnel para RDS via $BASTION_INSTANCE...${RESET}"
    ...
    log_message "Iniciando túnel RDS: $rds_endpoint:$rds_port via $BASTION_INSTANCE ($bastion_id) - Local:$local_port"
    ...
    local exit_code=$?
    if [ $exit_code -eq 0 ]; then
        log_message "Túnel RDS finalizado normalmente"
    else
        echo -e "${RED}[ERRO] Falha ao criar túnel RDS (código: $exit_code)${RESET}"
        log_message "Erro ao criar túnel RDS (código: $exit_code)"
    fi
}
...
# Verifica se o perfil AWS existe (apenas se PROFILE estiver definido)
